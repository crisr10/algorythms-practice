...
function anagrams(strA, strB) {
  const strAChars = stringMap(strA);
  const strBChars = stringMap(strB);

  if (cleanUpString(strA).length !== cleanUpString(strB).length) {
    return false
  } else {
...
function stringMap(str) {
  let mappedObject = {}
  for (let char of cleanUpString(str)) {
  ...
  // Created a helper function that creates a character map for a given string
